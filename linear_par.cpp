...
void l2r_lr_fun::XTv(double *v, double *XTv)
{
	...
	memset(XTv, 0, sizeof(double) * w_size);
	...
	double Cp, double Cn, int solver_type)
{
	...
	while (iter < max_iter)
	{
		...
		auto func = [&](int tid) {
			...
			double local_PGmax_new = -INF;
			double local_PGmin_new = INF;
			std::list<int> local_removal_list;
			...
			for (int s=start; s<end; s++)
			{
				...
				if (alpha[i] == 0)
				{
					if (G > PGmax_old)
					{
						local_removal_list.push_back(s);
						continue;
					}
					else if (G < 0)
						PG = G;
				...
				local_PGmax_new = max(local_PGmax_new, PG);
				local_PGmin_new = min(local_PGmin_new, PG);
...

			{
				...
				PGmax_new = max(PGmax_new, local_PGmax_new);
				PGmin_new = min(PGmin_new, local_PGmin_new);
				removal_list.splice(removal_list.end(), local_removal_list);
				for (int i = 0; i < w_size; ++i)
					tmpW[i] += local_w[i];
			}
		...
		{
			//std::cout << "PGmax_new " << PGmax_new << " - PGmin_new " << PGmin_new << " = ";
			//std::cout << PGmax_new - PGmin_new << std::endl;
			if(active_size == l)
				break;
			else
			{
		...
		if (PGmin_old >= 0)
			PGmin_old = -INF;
		//std::cout << "PGmax_old = " << PGmax_old << " PGmin_old = " << PGmin_old << std::endl;
	}
...
static void binary_svc_probability(
		const problem *prob, const parameter *param,
		double Cp, double Cn, double& probA, double& probB)
{
	...
	for(int i=0;i<nr_fold;i++)
	{
		...
		else
		{
			...
			struct model *submodel = train(&subprob,&subparam);
			for(int j=begin;j<end;j++)
			{
				predict_values(submodel,prob->x[perm[j]],&(dec_values[perm[j]]));
...
double predict_probability2(const struct model *model_, const struct feature_node *x, double* prob_estimates)
{
	if (model_->nr_class == 2 && model_->probA != NULL && model_->probB != NULL) 
	{
		int nr_class = model_->nr_class;
		...
		for(int i=0;i<nr_class;i++)
		{
			...
			{
				pairwise_prob[i][j]=std::min(std::max(sigmoid_predict(dec_values[k],model_->probA[k],model_->probB[k]),min_prob),1-min_prob);
		...
		free(dec_values);
		free(pairwise_prob);
		return model_->label[prob_max_idx];
	} else
		return 0;
}
...
struct model *load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		else if(strcmp(cmd,"probA")==0)
		{
			double probA;
			fscanf(fp,"%lf",&probA);
			model_->probA= Malloc(double, 1);
			model_->probA[0] = probA;
		}
		else if(strcmp(cmd,"probB")==0)
		{
			double probB;
			fscanf(fp,"%lf",&probB);
			model_->probB= Malloc(double, 1);
			model_->probB[0] = probB;
		}
...
int check_probability_model(const struct model *model_)
{
	// NEW: Probability estimate
	return (
			model_->param.solver_type==L2R_LR ||
